...
function registerTools(server, jobsAPI) {
    ...
    server.tool("apply-to-yadav_consulting-job", "Apply to Jobs where the job source_id is yadav_consulting", {
        jobId: zod_1.z.string().describe("The job ID to apply for"),
        applicant: zod_1.z.object({
            name: zod_1.z.string().describe("Full name of the applicant"),
            age: zod_1.z.string().describe("Age of the applicant"),
            gender: zod_1.z.string().describe("Gender of the applicant"),
            phone: zod_1.z.string().describe("Phone number of the applicant"),
            address: zod_1.z.string().describe("Complete Address of the applicant")
    ...
    server.tool("apply-to-rozgaar_setu-job", "Apply to Jobs where the job source_id is rozgaar_setu", {
        jobId: zod_1.z.string().describe("The job ID to apply for"),
        applicant: zod_1.z.object({
            name: zod_1.z.string().describe("Full name of the applicant"),
            age: zod_1.z.string().describe("Age of the applicant"),
            email: zod_1.z.string().describe("Email of the applicant"),
        }).describe("Applicant details")
    }, async ({ jobId, applicant }) => {
        const response = await jobsAPI.applyForJob(jobId, applicant);
        const successMessage = `
    ...
    server.tool("fetch-job-categories", "Fetch all the job categories available in the database", {}, async ({}) => {
        const searchRequest = { filters: {}, options: {} };
        const response = await jobsAPI.searchJobs(searchRequest);
        const jobs = response?.result?.jobs || [];
    ...
    server.tool("fetch-all-source-ids", "Fetch all the job source IDs available in the database", {}, async ({}) => {
        const searchRequest = { filters: {}, options: {} };
        const response = await jobsAPI.searchJobs(searchRequest);
        const jobs = response?.result?.jobs || [];
        const jobSourceIds = jobs.map((job) => job.source_id);
    ...
    server.tool("fetch-filtered-jobs", "Fetch jobs based on the given filters", {
        filters: zod_1.z.object({
            gender: zod_1.z.string().describe("The gender requirement for the job (e.g., male, female, any)").optional(),
            language: zod_1.z.string().describe("The language required for the job (e.g., Hindi, Kannada, English)").optional(),
            location: zod_1.z.string().describe("The location of the job (city, state, or address)").optional(),
        ...
        try {
            ...
            if (filters.location) {
                ...
                filterObj.$or.push({ 'location.city': { $regex: filters.location, $options: 'i' } }, { 'location.state': { $regex: filters.location, $options: 'i' } }, { 'location.complete_address': { $regex: filters.location, $options: 'i' } });
            }
    ...
    server.tool("get-application-status", "Get the status of a specific application", {
        jobId: zod_1.z.string().describe("The job ID to get the status for"),
