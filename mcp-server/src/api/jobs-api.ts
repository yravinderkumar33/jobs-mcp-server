...
export class JobsAPI {
   ...
   */
  async applyForJob(jobId: string, application: Record<string, any>): Promise<JobApplicationResponse> {
    try {
...
`;
  ...
  async getApplicationStatus(applicationId: string, jobId: string): Promise<JobApplicationResponse> {
    try {
      
      const response = await axios.get(`${this.baseUrl}/api/jobs/applications/${jobId}/${applicationId}/status`, {
        headers: {
          'Content-Type': 'application/json',
        }
      });
