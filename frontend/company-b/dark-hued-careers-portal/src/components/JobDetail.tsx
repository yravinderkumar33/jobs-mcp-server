    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-6">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="text-right">
              <div className="flex items-center gap-1 text-2xl font-bold text-green-500 mb-1">
                <IndianRupee className="h-6 w-6" />
                {job.salary.amount.toLocaleString()}
              </div>
              <p className="text-sm text-muted-foreground">per {job.salary.frequency}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <ul className="space-y-2">
                {job.responsibilities.map((responsibility, index) => (
                  <li key={index} className="flex items-start gap-2">
                    ...
                    <span className="text-card-foreground">{responsibility}</span>
                  </li>
                ))}
              </ul>
            </CardContent>
          ...
          <Card className="border-border">
            <CardHeader>
              <CardTitle className="text-foreground">About {job.company.name}</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-card-foreground mb-3">{job.company.description}</p>
              <Badge variant="secondary" className="bg-secondary text-secondary-foreground">Industry: {job.company.industry}</Badge>
            </CardContent>
          </Card>
        </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {job.requirements.education.map((edu) => (
                      <Badge key={edu} variant="outline" className="text-xs border-border">
                        {edu}
                      </Badge>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {job.requirements.languages.map((lang) => (
                      <Badge key={lang} variant="outline" className="text-xs border-border">
                        {lang}
                      </Badge>
                    ))}
                  </div>
          ...
          <Card className="border-border">
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-sm font-medium text-foreground">{job.schedule.shiftType}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Hours/Week:</span>
                <span className="text-sm font-medium text-foreground">{job.schedule.hoursPerWeek}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Weekend Work:</span>
                <span className="text-sm font-medium text-foreground">
              ...
              <div className="grid grid-cols-1 gap-2">
                {job.benefits.map((benefit) => (
                  <div key={benefit} className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-card-foreground">{getBenefitLabel(benefit)}</span>
                  </div>
                ))}
              </div>
            </CardContent>
